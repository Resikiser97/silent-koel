...
## Lang Key 命名規範（新增 UI 文字時必須遵守）

### 命名風格
- 使用 `camelCase`，放在 `lang/` 的 `ui` 物件下
- 不使用 `snake_case` 前綴（如 `btn_`、`ui_`）
...
| `key*` | 按鍵設定 |
| `achievement*` | 成就頁文字 |
| `stat*` | 屬性面板 label（statAtk / statHp / statBase 等） |
| `unit*` | 單位後綴（unitPerFruit / unitPerKill 等） |

### 不做 i18n 的例外（跳過，不建立 lang key）
以下屬於 UI icon / 格式符號，不是語言文字，直接留在代碼：
- 通用符號：`✕` `◀` `▶` `✓` `←` `→` `🏆` `？` `—`
- 公式符號：`' + '` `' ×'` `' ▾'`
- 暫時性 roadmap 文字（含「待 v0.x.x」說明）

### 新增 lang key 流程
1. 先確認 `lang/` 現有 key，避免重複或語意衝突
2. 同時修改 `lang/zh-TW.js` 與 `lang/en.js`，兩份必須同步
3. 再修改引用處改為 `t('key')`
4. 不確定歸屬前綴時，先回報開發者確認，不要猜
