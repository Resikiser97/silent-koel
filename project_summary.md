...
### 進化系統（各路線 Lv1~5）
- 🌿 草食性：HP 加強、果子 XP、中立生物友善（Lv4/5 狂暴狀態也不攻擊）；初始已解鎖 Lv1；**未達草食性 Lv1 時，吃果子只得 1 XP**（v0.57.6，避免靠果子觸發升級回血）
...
- v0.47.1 新增：操作說明分頁擴充至 5 頁
  - 第4頁「Boss 圖鑑」（`_buildBossPage()`）：動態引用 EASY_MAP/NORMAL_MAP bosses，顯示簡單/普通兩套數值、普通難度技能說明（黑熊/大白鯊/蠍王）、通用回血、弱點提示
  - 第5頁「難度介紹」（`_buildDifficultyPage()`）：動態引用地圖 config，顯示生物強度倍率、精英/Boss 獎勵、特殊機制開關
- v0.47.1 新增：`buildEvoLevelDesc(pathId, upToLevel)` 全域函式，動態生成進化圖鑑各等級描述，config 改數值自動同步
