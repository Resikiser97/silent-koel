...
**當前狀態：** 困難模式多項 Bug 修復完成（巨人 AI、難度保留根本原因修正、黑色獵人血管判斷），準備進入系統重構階段。

**下一步：**
1. 測試 v0.1.14.5 困難模式再來一場難度保留
2. Stage D：中層系統重構
3. Stage F：核心循環依賴打破
...
### 最近完成的工作
- v0.1.14.5：修復 main.js window.onload autostart 路徑未設 currentMap 的根本原因，困難模式再來一場現在正確使用 HARD_MAP
