...
systems/creatures.js      _PACK_NAMES / _usedPackNames / resetPackNames()（草食巨人隊伍名稱池，v0.0.66.2；v0.0.68.0 改仿製詞）
                          ...
                          _tryMeleeAttack / _bodyMeleeRange / _drawAttackTelegraph（近戰前搖/命中/後搖共用流程；命中距離公式為半徑 + max(attackRange, 目標半徑) + rangeBuffer，v0.1.26.1）
                          _applyCreatureSeparation（通用生物分離：肉食防重疊、草食可近距離抱團、巨人高權重推開，v0.1.26.0）
                          notifyCreatureHitByPlayer（玩家命中生物事件：巨人/Alpha 5000ms 激怒轉玩家、鬣狗本地 pack focus 集火玩家，v0.1.26.0）
                          _getHyenaLocalPackCount / _selectHyenaSurroundAttackers（鬣狗本地隊伍計數與 4+ 包圍雙攻擊者選擇，v0.1.26.0）
...
systems/elite.js          spawnEliteCreature, updateEliteCreature, drawEliteCreature, drawEliteArrow
                          三犬精英近戰前搖/命中/後搖提示與體型命中範圍（同 _bodyMeleeRange 公式，v0.1.26.1）
...
systems/chat.js           _sha256, loadChatSettings, saveChatSettings
                          ...
                          _handleMovePinCommand（/movepin <from> <to>：to 空→移動+補位；to 有 pin→兩格互換，v0.1.28.1）
